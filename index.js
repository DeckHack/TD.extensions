...
TD.extensions.checkDependencies = (extensionName) => {
  ...
  let extensionDependencyStatus = extension.dependencies.every((dependency) => TD.extensions.isEnabled(dependency))
    ...
    if (extension.dependencies) {
      extension.dependencies.forEach(function (dependency) {
        if (ancestors.indexOf(dependency) >= 0) {
          throw new Error('Circular dependency "' + dependency + '" is required by "' + extension.name + '": ' + ancestors.join(' -> '))
        }
